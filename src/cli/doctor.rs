//! `uniclaw doctor` - active health diagnostics.
//!
...
async fn check_nearai_session() -> CheckResult {
    ...
    if !session_path.exists() {
        ...
        return CheckResult::Fail(format!(
            "session file not found at {}. Run `uniclaw onboard`",
...
mod tests {
    ...
    fn check_binary_skips_nonexistent() {
        match check_binary("__uniclaw_nonexistent_binary__", &["--version"]) {
            CheckResult::Skip(_) => {}
            other => panic!(
