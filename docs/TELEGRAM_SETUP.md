...
This guide covers configuring the Telegram channel for UniClaw, including DM pairing for access control.

## Overview

The Telegram channel lets you interact with UniClaw via Telegram DMs and groups. It supports:
...
- UniClaw installed and configured (`uniclaw onboard`)
...
2. Bot replies: `To pair with this bot, run: uniclaw pairing approve telegram ABC12345`
3. You run: `uniclaw pairing approve telegram ABC12345`
...
uniclaw pairing list telegram

# List as JSON
uniclaw pairing list telegram --json

# Approve a user by code
uniclaw pairing approve telegram ABC12345
```

### Configuration

Edit `~/.uniclaw/channels/telegram.capabilities.json` (or the config injected by the host):
...
- **Secrets store**: `uniclaw` CLI (if available)
...
- Set `bot_username` in config to your bot's username (e.g., `MyUniClawBot`)
- Ensure the message contains `@YourBot` or starts with `/`

### "Connection refused" when starting

- For webhook mode: Start your tunnel before `uniclaw run`
